...
- **Thread Safety**: Method invocations on `queue()`, `size()`, `addCounter()`, `removeCounter()`, and `toJson()` are thread-safe.
- **Exceptions**: Callback exceptions are caught locally without terminating background worker threads.
- **Diagnostics**: `.toJson()` methods return `nlohmann::json` objects containing snapshot telemetry.
